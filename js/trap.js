...
class Trap {
    ...
    applyEffect(target, effect, game) {
        ...
        switch (effect.type) {
            ...
            case 'toxic_swamp':
                ...
                this.applyToxicSwampEffect(target, effect);
    ...
    applyPoisonEffect(target, effect) {
        ...
        const poisonEffect = new StatusEffect(
...

            {
                ...
                maxStacks: effect.dot.maxStacks || effect.maxStacks || (STATUS_EFFECT_CONSTANTS?.POISON_MAX_STACKS ?? 4),
    ...
    applyToxicSwampEffect(target, effect) {

        // 毒沼効果: 強力な毒DOT + 移動不可(ルート)
        ...
        const poisonEffect = new StatusEffect(
...

            {
                ...
                maxStacks: effect.dot.maxStacks || 5,
        ...
        if (effect.root) {
            ...
            const rootEffect = new StatusEffect(

                'rooted',

                effect.root.duration,

                {

                    slow: 1.0, // 100%減速 = 移動不可

                    breakOnDamage: effect.root.breakOnDamage || false

                }

            );

            target.statusEffects.addEffect(rootEffect);
