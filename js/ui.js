...
class UIManager {
    ...
    showHelp() {
        ...
        p1_1.textContent = '15ウェーブの侵入者から魔法のコアを守り抜くこと！';
        ...
        ['🟢 明るい緑色 (S): スポーン地点 - 敵が出現する場所',
         ...
         '🟣 紫色: 高台 - 飛行モンスター専用の配置場所',
         '⬛ 濃い黒色: 溝 - 敵を押し出すと落下ダメージ'].forEach(text => {
        ...
        h3_9.textContent = '🗺️ マップパターン';
        const ul9 = document.createElement('ul');
        ['ストレート: 一本道の基本マップ',
         'ジグザグ: 上下に蛇行する経路',
         'スプリット: 上下に分岐する経路。敵は安全なルートを選ぶ！',
         'メイズ: 迷路風の複雑な経路',
         '※ ゲーム開始時にランダムに選択されます',
         '⚠️ 重要: 敵は罠の配置に応じて経路を変更します'].forEach(text => {
            const li = document.createElement('li');
            li.textContent = text;
            ul9.appendChild(li);
        });
